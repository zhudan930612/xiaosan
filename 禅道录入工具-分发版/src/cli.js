...
import { ensureDirectory, loadConfig, saveUserConfig } from './importer/config.js';
...
export function mergeRuntimeConfig(config, runtimeInput) {
  return {
    ...config,
    baseUrl: (runtimeInput.baseUrl || config.baseUrl || '').trim(),
    account: (runtimeInput.account || config.account || '').trim(),
    password: runtimeInput.password || config.password || '',
  };
}
...
async function promptForMissingConnectionConfig(config) {
  const missingBaseUrl = !String(config.baseUrl || '').trim();
  const missingAccount = !String(config.account || '').trim();
  const missingPassword = !String(config.password || '').trim();
  if (!missingBaseUrl && !missingAccount && !missingPassword) {
  ...
  try {
    console.log('\n首次使用请填写你自己的禅道登录信息。');
    const runtimeInput = {
      baseUrl: missingBaseUrl ? (await rl.question('禅道地址（例如 http://120.46.219.85:8004/zentao）: ')).trim() : '',
      account: missingAccount ? (await rl.question('禅道账号: ')).trim() : '',
      password: missingPassword ? (await rl.question('禅道密码: ')).trim() : '',
    };

    const mergedConfig = mergeRuntimeConfig(config, runtimeInput);
    if (!mergedConfig.baseUrl) throw new Error('禅道地址不能为空');
    if (!mergedConfig.account) throw new Error('禅道账号不能为空');
    if (!mergedConfig.password) throw new Error('禅道密码不能为空');

    const shouldSaveAnswer = (await rl.question('是否保存到当前目录的 config.local.json，供下次直接使用？输入 y 保存: ')).trim().toLowerCase();
    const shouldSave = shouldSaveAnswer === 'y' || shouldSaveAnswer === 'yes';
    if (shouldSave) {
    ...
    return { config: mergedConfig, saved: shouldSave };
  } finally {
    rl.close();
  }
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const { config, saved } = await promptForMissingConnectionConfig(loadedConfig);
  await ensureToolArtifacts(config);
  if (saved) console.log(`已保存个人登录配置: ${config.localConfigPath}`);
